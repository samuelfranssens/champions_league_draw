...
awayteams <- c(A1,B1,C1,D1,E1,F1,G1,H1)
hometeams <- c(A2,B2,C2,D2,E2,F2,G2,H2)
...
constraints2 <- matrix(data=NA,nrow=9,ncol=2)   # adjust nrow to the amount of constraints you put in below
constraints2[1,] <- c("psg","monaco")
constraints2[2,] <- c("city","arsenal")
constraints2[3,] <- c("city","leicester")
constraints2[4,] <- c("bayern","dortmund")
constraints2[5,] <- c("leverkusen","dortmund")
constraints2[6,] <- c("real","barcelona")
constraints2[7,] <- c("real","atletico")
constraints2[8,] <- c("sevilla","barcelona")
constraints2[9,] <- c("sevilla","atletico")
