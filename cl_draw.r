...
knockoutphase <- length(awayteams) # which knockoutphase of the knockout phase is it ?

# create a matrix for games that are not allowed
# constraints1 = teams from same group
constraints1 <- matrix(data=NA,nrow=knockoutphase,ncol=2) 
...
draw <- matrix (data=NA,nrow=simulations,ncol=knockoutphase*2+amountofconstraints+knockoutphase) 
...
firstconstraintcolumn <- knockoutphase*2+1
lastconstraintcolumn  <- knockoutphase*2 + amountofconstraints
...
while (k < simulations+1) { # start a loop
  ...
  draw[k,] <- c(sample(hometeams,knockoutphase,replace=F),sample(awayteams,knockoutphase,replace=F),numeric(amountofconstraints+knockoutphase))
  ...
  for (j in 1:amountofconstraints) {  # go through each constraint
    if (violation_already == 0) {     # as long as there is not already a violation 
      for (i in 1:knockoutphase){     # go through each game in the draw
        if (draw[k,i] == constraints[j,1] & draw[k,i+knockoutphase] == constraints[j,2]) # check whether a game is disallowed
          {
          draw[k,knockoutphase*2+j] <- 1 # if constraint j is violated
          violation_already <- 1
          } 
        }
      }
...
for (l in 1:knockoutphase)  {                               # go through each home team and check who is the opponent
 opponent <- which(draw[k,]==hometeams[l]) + knockoutphase  # who is home team l's opponent?
 correctcolumn <- knockoutphase*2+amountofconstraints+l     # what is home team l's column?
...
probabilities <- matrix(data=NA,nrow=knockoutphase,ncol=knockoutphase)

for (i in 1:knockoutphase)   # for each home team
{
  for (k in 1:knockoutphase) # for each away team
  {
probabilities[k,i] <- length(which(draw[,knockoutphase*2+amountofconstraints+i]==awayteams[k]))/simulations*100
  }
}
